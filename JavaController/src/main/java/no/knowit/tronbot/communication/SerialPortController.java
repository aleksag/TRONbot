...
public class SerialPortController implements Runnable, SerialPortEventListener {
    ...
    private static final String PORT_NAMES[] = { 
        "/dev/tty.usbserial-A6008lIf", // Mac OS X
        "/dev/ttyUSB0", // Linux
        "COM12", // Windows
    };
    ...
    private void sendMessage(Message msg) throws IOException {
        output.write(protocol.getMessageBytes(msg));
    }
    ...
    public synchronized void serialEvent(SerialPortEvent oEvent) {
        if (oEvent.getEventType() == SerialPortEvent.DATA_AVAILABLE) {
            try {
                receiveMessage(input.available());
            } catch (Exception e) {
    ...
    private void receiveMessage(int availableBytes) throws IOException {
        byte chunk[] = new byte[availableBytes];
        input.read(chunk, 0, availableBytes);
        Message msg = protocol.getMessage(chunk);
        msgController.ackReceived(msg.getId());
        msgController.receiveMessage(msg);
    }
}
